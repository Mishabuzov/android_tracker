...
public class CircularProgressBar extends ProgressBar {
    ...
    private int mProgressColorId = 0;
    ...
    public void init(AttributeSet attrs, int style) {
        ...
        if (mProgressColorId != 0) {
            mProgressColorPaint.setColor(ActivityCompat.getColor(getContext(), mProgressColorId));
        } else {
            color = a.getString(R.styleable.CircularProgressBar_cpb_progressColor);
            if (color == null)
                mProgressColorPaint.setColor(ActivityCompat.getColor(getContext(), R.color.circular_progress_default_progress_line));
            else
                mProgressColorPaint.setColor(Color.parseColor(color));
        }

        if (mBackgroundColorId != 0) {
            mBackgroundColorPaint.setColor(ActivityCompat.getColor(getContext(), mBackgroundColorId));
        } else {
            color = a.getString(R.styleable.CircularProgressBar_cpb_backgroundColor);
            if (color == null)
                mBackgroundColorPaint.setColor(ActivityCompat.getColor(getContext(), R.color.circular_progress_default_progress_line));
            else
        ...
        if (color == null)
            mTitlePaint.setColor(ActivityCompat.getColor(getContext(), R.color.circular_progress_default_progress_line));
        else
            mTitlePaint.setColor(Color.parseColor(color));

        color = a.getString(R.styleable.CircularProgressBar_cpb_subtitleColor);
        if (color == null)
            mSubtitlePaint.setColor(ActivityCompat.getColor(getContext(), R.color.circular_progress_default_progress_line));
        else
        ...
        mStrokeWidth = a.getInt(R.styleable.CircularProgressBar_cpb_strokeWidth, mStrokeWidth);
        a.recycle();
    ...
    public synchronized void setProgressColor(int id) {
        mProgressColorId = id;
        invalidate();
    }

    public synchronized void setBackgroundColor(int id) {
